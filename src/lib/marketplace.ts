...
/** One node in the hero's verification floor. */
export interface FloorNode {
  name: string;
  chainId: number;
  tokenId: string;
  category: Category;
  /** 0..1, or null when never probed. */
  uptime: number | null;
  probes: number;
  attestations: number;
}
...
/**
 * A sample of indexed agents with what we have measured about each.
 *
 * Feeds the landing page's verification floor. Real agents and real probe
 * results, because a hero illustration of agents being checked would be an odd
 * place to start inventing them — and because it then doubles as a live status
 * board rather than decoration.
 */
export async function listFloor(limit = 16): Promise<FloorNode[]> {
  const agents = await listSearchable({ limit: 6 });

  // An agent can be surfaced by more than one category query, and the same
  // token appearing twice would collide as a React key and render one tile
  // twice while dropping another.
  const seen = new Set<string>();
  const unique = agents.filter(({ listing }) => {
    const key = `${listing.agent.chain_id}:${listing.agent.token_id}`;
    if (seen.has(key)) return false;
    seen.add(key);
    return true;
  });

  return unique.slice(0, limit).map(({ listing, record }) => ({
